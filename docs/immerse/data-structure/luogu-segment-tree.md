...
### 分析
这题最值得建立的是“为什么区间修改不能每次都递到叶子”的直觉。因为题目只问区间和，所以当一个结点代表的整段都被 `[l,r]` 完全覆盖时，其实没必要立刻去改每个单点；只要知道这整段所有元素都一起加了 `k`，区间和就已经能立刻更新成 `sum + k * len`。

于是懒标记的含义就很清楚了：`tag` 不是“还没做的操作”，而是“这整段已经统一加过、只是还没分发给孩子”。只有当后续查询或修改真的要深入子区间时，才把这份统一增量下传。

所以这道模板题真正该记住的是：**如果整段信息能直接更新，就先改结点本身，把细节延后。**
### 核心代码
```cpp
...
### 分析
这题表面上有“乘”和“加”两种修改，实际上它们都属于同一类操作：把每个元素做一次仿射变换 `x -> x * mul + add`。一旦看出这一点，线段树里就不该再分别想“乘法标记”和“加法标记”，而该想“当前整段还要再套一个什么函数”。

节点维护区间和很好理解，因为仿射变换作用到整段时，`sum` 也会整体按 `sum * mul + add * len` 更新。真正容易写错的是标记复合顺序：若父亲先乘后加，儿子原本也有自己的变换，那两者是函数复合，不是简单地各自独立存着。

所以这题最关键的不是线段树本身，而是先把操作抽象成一个统一函数：**多种区间修改能否写成同一种懒标记复合。**
### 核心代码
```cpp
...
### 分析
这题和上一道模板二本质完全相同，关键不是重新背一遍代码，而是认出：题目虽然换了说法，但维护对象还是“区间和 + 仿射懒标记”。

一旦区间操作同时包含乘和加，就该立刻想到把它们合并成 `x -> x * mul + add`。之后所有难点都归结到同一件事：父节点的变换如何正确叠到儿子已有标记上，也就是函数复合顺序不能写反。

所以这题更像模板二的“识别训练题”：**看到乘加同存，就先想仿射；看到仿射，就想到两个懒标记一起维护。**
### 核心代码
```cpp
void apply(int p, int l, int r, long long mul, long long add) {
    tr[p].sum = (tr[p].sum * mul + add * (r - l + 1)) % mod;
...
### 分析
直接在原数组上维护等差数列区间加会很别扭，因为一次操作在区间内部不是统一增量。真正的突破口是先换维护对象：把数组变成差分数组后，等差数列的“变化率”就会变得非常规整。

具体地说，首项 `k`、公差 `d` 的等差段加到 `[l,r]` 上，只会让差分数组在很少几个位置发生规则变化：开头跳一下、中间保持常数 `d`、结尾再补一个反向收束。于是原题里复杂的“区间加等差数列”，被翻译成了差分数组上的少量区间 / 单点修改。

查询某个位置值时，再把差分做前缀和还原即可。所以这题最该记住的是：**当区间修改内部不统一时，先问差分后会不会变简单。**
### 核心代码
```cpp
...
### 分析
区间取模看起来不像线段树操作，因为它既不是加法也不是赋值，没有简单的懒标记可打。真正能救它的，是取模操作的“无效区间”很好判断：若一整段最大值都小于 `x`，那么这一段所有数 `% x` 后完全不变。

于是线段树该维护的重点就不是标记，而是**这一段还有没有必要继续递归**。只要结点里存区间和和区间最大值，遇到 `mx < x` 就整段剪掉；只有那些仍可能发生变化的区间才下探到更细层。

所以这题最重要的迁移点是：**当区间操作无法懒标记时，试着找一个足够强的“剪枝判据”。**
### 核心代码
```cpp
...
### 分析
这题和区间取模很像，表面上没有好用懒标记，但操作本身具有很强的收敛性。因为一个数不断变成约数个数后，会很快掉到 `1` 或 `2`，之后再操作就不会继续变化。

这说明判断“这段还值不值得递归”非常容易：若结点最大值已经 `<= 2`，整段更新都是空操作。于是线段树仍然维护区间和与最大值，修改时靠最大值剪枝，不必硬想一个不存在的懒标记。

所以这题和取模题一起很适合总结成一类：**操作本身不可合并，但数值会快速稳定，于是靠最大值 / 最小值剪枝。**
### 核心代码
```cpp
int divs[MAXA];
struct Node { long long sum; int mx; } tr[N << 2];
...
### 分析
区间开方也不是懒标记能直接承接的操作，因为不同元素开方后的变化量不同。但它和区间取模、约数个数替换有同一个优点：重复作用会迅速收敛。

只要一段最大值已经 `<= 1`，这段再怎么开方都不会变化，因此可以整段剪枝。剩下真正需要下探的，只是那些还大于 `1` 的位置。节点维护区间和与最大值，就足够同时支持查询与剪枝。

所以这题再次强化了一个线段树思路：**不是所有区间题都靠懒标记，有些题靠“很快稳定”来剪枝。**
### 核心代码
```cpp
...
### 分析
这题最适合帮助建立“什么信息能在线段树里合并”的判断。因为 `gcd` 满足结合律，`gcd([l,r])` 可以拆成左半区间和右半区间的 `gcd` 再合并。

一旦区间答案满足这种可分治合并的性质，线段树就只需要在每个结点存整段的 `gcd`。查询时把覆盖到的若干结点结果再做几次 `gcd` 即可，完全不需要任何懒标记。

所以这题的价值不在于难度，而在于帮你快速识别：**只查不改，且答案可结合合并，就能直接上静态线段树。**
### 核心代码
```cpp
...
### 分析
这题和前面的区间开方是同模版，只是数据范围更大，因此更能体现剪枝的重要性。若没有“这段已经稳定”的判断，递归次数会爆得很厉害。

核心仍然是结点最大值：只要 `mx <= 1`，继续开方就是空操作，可以立刻返回。真正要修改的只会是那些还没稳定的叶子，而每个数被真正更新的次数其实很有限。

所以这题值得和前面的开方题一起记：**大数据范围下，最大值剪枝不是优化细节，而是做法成立的前提。**
### 核心代码
```cpp
struct Node { long long sum; int mx; } tr[N << 2];
...
### 分析
如果 `n` 只有几百万，普通线段树模板就够了；但这里 `n` 到了 `10^9`，真正困难已经不是操作本身，而是树根本没法一次性开完。于是要先换一个视角：序列虽然长，但被访问到的区间其实只和操作数有关。

动态开点线段树正是为这种“值域大、访问稀疏”的场景准备的。只有当某段真的被修改或查询到时，才创建对应结点；没碰过的整段默认仍保持初始值。这样空间复杂度就不再和 `n` 挂钩，而是近似跟访问过的结点数量挂钩。

所以这题最核心的选型判断是：**值域巨大但访问稀疏时，不要硬建整棵树，改用动态开点。**
### 核心代码
```cpp
...
### 分析
01 翻转题最关键的是想清楚“翻转”对结点信息会造成什么影响。若一个区间长度是 `len`，原来有 `sum` 个 `1`，翻转后就一定变成 `len-sum` 个 `1`，根本不需要知道每个单点的具体值。

这说明翻转操作非常适合懒标记：整段被覆盖时，直接把 `sum` 改成 `len-sum`，再把翻转标记异或一下即可。之所以标记能异或合并，是因为连续翻转两次等于没翻，天然就是模 `2` 叠加。

所以这类题最该记住的是：**先问一次区间操作对结点统计量有没有直接公式，有就优先打懒标记。**
### 核心代码
```cpp
...
### 分析
这题和 01 串翻转没有本质区别，唯一变化只是题面从字符换成了灯泡。真正该训练的是模型识别：只要状态只有 `0/1` 两种，区间操作又是整段取反，就应该立刻想到“亮灯数 = 区间长度 - 原亮灯数”。

于是线段树仍然只需维护亮灯数和翻转标记。整段翻转时直接更新统计量，懒标记用异或叠加；查询时按普通区间和处理即可。

所以这题的重要性不在新技巧，而在帮助你形成模板迁移：**看见灯泡翻转，就该像看见 01 串翻转一样快。**
### 核心代码
```cpp
...
### 分析
这又是一道标准 01 翻转题，重点依然不在写法，而在把题目迅速压回同一个模板。区间翻转不会改变“这段有多少个元素”的长度信息，只会把开和关的数量互换，所以结点统计量仍然只用维护开启个数即可。

懒标记之所以是一个布尔 `rev`，本质上就是因为翻转操作可逆且满足两次抵消。整段翻转只改 `sum` 和 `rev`，继续下探时再把它传播给孩子。

这题很适合作为统一记忆点：**一切 01 状态的整段取反，优先想 `sum = len - sum`。**
### 核心代码
```cpp
...
### 分析
这题几乎是“线段树能不能上”的最基础判断题。因为区间最小值满足可结合合并：整段最小值一定等于左半最小值和右半最小值中的较小者。

所以每个结点只需保存自己区间的最小值，建树时自底向上 `pull`，查询时把覆盖到的若干结点结果再做几次 `min` 即可。没有修改时，它和 ST 表、Sparse Table 属于同一类 RMQ 思路，只是线段树更通用。

这题的价值主要在于打牢一个判断：**只查区间值，且答案能由左右子区间同类信息合并出来，就能直接上静态线段树。**
### 核心代码
```cpp
...
### 分析
这题最难的不是区间赋值，而是“最靠左能放下长度为 `x` 的连续空房在哪”。如果只维护空房总数，完全不够，因为连续性信息会丢失。

因此结点里真正该存的是三件事：前缀最长空房长度、后缀最长空房长度、区间内部最大连续空房长度。这样左右儿子一合并，就能知道一段连续空房是不是跨过中点形成的；查找最靠左答案时，也能按“先看左边，再看跨中点，最后看右边”的顺序递归。

所以这题最值得形成的模板感是：**当题目既问区间赋值又问最长连续段时，节点信息通常就是前缀 / 后缀 / 区间最优三件套。**
### 核心代码
```cpp
...
### 分析
矩形面积并最怕重复计算重叠部分，所以不能按矩形一块块加。更自然的做法是沿 `x` 轴扫：相邻两条竖边之间，当前被覆盖的纵向总长度若为 `len`，那么这一整条竖条贡献的面积就是 `len * dx`。

于是问题被降成一个动态区间覆盖：扫描线每经过一条左边界，就把对应纵区间覆盖次数 `+1`；经过右边界，就 `-1`。线段树维护的核心量不是“最大值”而是“当前纵轴上被至少覆盖一次的总长度”。只要某段覆盖次数 `cnt > 0`，这整段长度就可以直接算满。

所以这题真正要掌握的是扫描线和线段树的分工：**扫描线负责决定当前有哪些矩形生效，线段树负责把这些生效区间压成一条总覆盖长度。**
### 核心代码
```cpp
...
### 分析
这题容易被“不断切割”误导去想复杂区间结构，其实真正变化的只有一件事：某次切割只会把当前某一段长度 `L` 删除，再新增两段长度 `a` 和 `L-a`。

所以维护的重点根本不是整块玻璃，而是横向、纵向各自被切成了哪些段。用有序集合找到新切点左右相邻的旧边界，就能知道被切开的原段长度；再用 multiset 维护所有段长，随时取最大横段和最大竖段，相乘就是答案。

这题很适合提醒自己：**有些区间题看起来像线段树，实则更像“维护切点 + 维护段长集合”。**
### 核心代码
```cpp
set<int> sx, sy;
multiset<int> wx, wy;
...
### 分析
单棵线段树之所以不够，是因为题目同时有两维：一维是下标区间 `[l,r]`，一维是值域上的排名 / 第 `k` 小。只在下标上建树，结点里就必须再知道“这一段里所有数的有序分布”。

这正是树套树的来源：外层线段树负责把查询区间拆成 `O(log n)` 个结点，内层平衡树 / 有序结构负责在这些结点里统计 `<= x` 的个数。于是排名问题变成计数问题，第 `k` 小则可以再对值域二分；前驱后继也都能归到同一类值域统计。

所以这题最重要的建模点是：**当一个问题同时卡在“区间”和“值域顺序”两层上时，往往需要一层结构管位置、另一层结构管值。**
### 核心代码
```cpp
...
### 分析
这题先别想着“选哪些牛”，而要先判断贪心顺序。按右端点从小到大处理是自然的，因为越早结束的请求越不该被后面更长的区间占掉资源，这和经典区间调度的直觉一致。

一旦顺序确定，判断某头牛能不能接纳就只剩一句话：它覆盖的所有畜栏剩余容量最小值是否还大于 `0`。若可以，就把整段容量统一减一。于是问题被翻译成“区间最小值查询 + 区间减法”，正好落在线段树上。

所以这题最值得迁移的是两步法：**先用贪心确定处理顺序，再让线段树承接资源是否足够的判定。**
### 核心代码
```cpp
...
### 分析
李超线段树处理的不是普通区间信息，而是一组函数的上凸壳 / 下凸壳查询。对固定的 `x` 而言，我们只关心哪条线在这里最优，所以一个结点里没必要存很多线，只需要保留“在这个区间里最有资格竞争的一条候选线”。

普通李超树是全局插整条直线；而这题更进一步，每条直线只在某个 `x` 区间内有效，因此要先把它限制在覆盖到的节点上，再在这些节点里做李超比较。比较逻辑的核心仍然不变：看中点谁更优，把暂时劣势的线丢去可能翻盘的那一侧继续递归。

所以这题真正该学的是：**当维护对象从“数”变成“函数”时，线段树的每个结点也就不再存区间答案，而是存区间上的最佳候选函数。**
### 核心代码
```cpp
...
### 分析
这题是李超线段树最纯的原型：每个方案都是一条直线，查询某一天的最优收益，其实就是问“所有已加入直线在横坐标 `x` 处谁最大”。

如果把所有直线都存下来暴力比较，每次查询会很慢；而李超树利用了直线之间“区间上优劣只会切换有限次”的性质。一个结点只保留当前区间内更占优势的那条线，另一条线若还有机会赢，只会在左半或右半某一侧继续递归。

所以这题最值得建立的是函数视角：**线段树划分的是横坐标区间，结点竞争的是哪些直线在这段区间里更有希望成为最优。**
### 核心代码
```cpp
...
### 分析
这题虽然叫“平衡树模板”，但如果值域可离散化，其实完全可以从另一条思路做：把每个值出现了多少次看成值域上的频次数组，然后在线段树上维护这些频次。

这样排名、本质上就是“严格小于它的元素个数 + 1”；第 `k` 小则是在频次数组上找第 `k` 个 `1`（或第 `k` 个计数单位）；前驱后继也都转成值域上往左 / 往右找最近非零位置。可重集合在这里不再是一棵按值旋转的树，而是一段按值统计的区间。

所以这题很适合作为桥梁：**很多看似必须用平衡树的顺序统计题，只要值域能控，也能改写成权值线段树。**
### 核心代码
```cpp
...
### 分析
这题的核心不是查询，而是“集合本身会被拆开又并回去”。普通平衡树做合并 / 分裂常见，但这里按的是**值域区间**去拆，因此最自然的结构反而是权值线段树。

每个集合对应一棵值域线段树，叶子记录某个值出现多少次。若要把 `[x,y]` 这段值域拆出去，只需递归找到覆盖这段值域的节点，把它们从原树摘下挂到新树上；反过来合并两个集合时，再把对应值域的计数递归相加。

所以这题真正该掌握的是：**当操作直接发生在值域区间上时，集合的“内容”本身就该用一棵值域树来表示。**
### 核心代码
```cpp
...
### 分析
加强版真正增加的不是思路难度，而是把你逼到必须彻底理解“为什么矩阵化动态 DP 成立”。静态树上最大权独立集有 `f[u][0/1]` 两态，而重链上一段父子转移可以被抽象成一个小矩阵；整条链的信息就是这些矩阵按顺序相乘的结果。

因此单点改权后，不需要重做整棵树，只需修改受影响点对应的基矩阵，再沿重链把若干段矩阵乘积重新维护上去。这个核心逻辑和 P4719 完全一样，只是这里数据更强，要求你把树剖、线段树和矩阵乘法都写得足够紧。

所以这题最重要的认识是：**加强版不是新模型，而是逼你把“点改动 -> 链矩阵变动 -> 全局 DP 更新”这条传递链真正吃透。**
### 核心代码
```cpp
