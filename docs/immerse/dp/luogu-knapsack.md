...
题目表面在问“剩余空间最小”，但背包题更习惯优化“装进去多少”。因为箱子容量固定，剩余空间最小等价于已装总体积最大。

于是只要把每个物品的“价值”也设成它自己的体积，问题就被拉回最普通的 01 背包：每件物品选或不选一次，目标是在容量不超过 `V` 的前提下，让总体积尽量接近 `V`。

所以这题很值得建立一个转换意识：**当目标是“剩得最少”时，往往可以先改写成“装得最多”。**
...
这题虽然包装成“价格 + 重要度”，但真正被优化的量其实已经直接给出来了：买一件物品的收益就是 `价格 × 重要度`。

因此它和标准 01 背包没有本质区别：容量是预算，物品费用是价格，价值就是 `v×w`。因为每件物品最多买一次，所以内层必须逆序枚举预算，防止同一件物品在一轮内被重复使用。

这题的迁移点很简单但很重要：**先把题面里真正要最大化的收益量翻译成物品价值，再看它是不是标准背包。**
...
这是最纯粹的 01 背包原型题：每件物品只有“拿”或“不拿”两种决定，而且容量约束只有一维。

定义 `f[j]` 表示容量为 `j` 时的最大价值，枚举当前物品后，从大到小更新容量。逆序的意义在这里尤其值得强调：它确保转移使用的是“上一轮、还没放当前物品”的状态，因此每件物品只会生效一次。

所以这道模板题真正该记住的是 01 背包的核心判据：**物品一次性、状态是一维容量、更新必须逆序。**
...
这题和装箱问题本质完全一样：限制是总重量不超过 `H`，目标是让吃掉的总重量尽量大。

因此最自然的翻译仍然是“体积即价值”的 01 背包。每捆干草要么吃要么不吃一次，状态 `f[j]` 记录承重不超过 `j` 时能达到的最大总重量，最后直接看 `f[H]`。

所以它其实是在反复巩固同一个技巧：**当每件物品的收益正好等于消耗本身时，本质就是在尽量把容量塞满。**
...
完全背包和 01 背包只差一个本质限制：这里同一种物品可以重复拿很多次。

这会直接反映在循环顺序上。若对当前物品正序枚举容量，那么更新 `f[j]` 时用到的 `f[j-v_i]` 可能已经在本轮被当前物品更新过，等价于允许再拿一次当前物品，这恰好符合“无限次选取”。

所以完全背包最重要的不是背模板，而是记住这句对照：**01 背包逆序防重复，完全背包正序保重复。**
...
这题和“采药”只有一个关键变化：草药不再只能采一次，而是可以无限摘取。因此模型立刻从 01 背包切到完全背包。

状态仍然是一维容量 `f[j]`，只是内层要改成正序枚举时间，表示当前草药可以在同一轮里被重复利用。题面里真正容易忽略的点反而是数值范围：总价值会变大，所以答案数组要用 `long long`。

所以这题适合用来强化一个切换信号：**“最多一次”改成“无限次”时，状态不一定变，循环方向先变。**
...
“每种题可以做任意多道”这句话，就是完全背包最明显的触发器。因为同一种题型可以被重复选择，限制资源只有总时间。

于是直接把耗时看成体积、得分看成价值，状态 `f[j]` 表示总时间不超过 `j` 时的最高分。正序枚举时间，就等价于允许当前题型在本轮中反复被使用。

这题虽然简单，但很适合帮助读者把题面语言和模型快速对上：**可无限次选择同类决策 → 完全背包。**
...
分组背包和普通 01 背包最大的区别，是冲突关系从“每件物品最多一次”升级成了“同组里最多选一件”。因此不能再逐件独立更新，否则同一组可能会被选出两件。

正确做法是按组推进：处理一整组时，对每个容量 `j`，枚举这组里到底选哪一件，或者一件都不选。容量依然要逆序枚举，因为每组本质上仍然只允许做一次决策。

所以分组背包最重要的状态意识是：**“组”才是当前阶段，组内物品只是这一阶段的若干备选项。**
...
二维费用背包其实没有换思想，只是“资源”从一维容量变成了两维：这里同时受体积和重量约束。

因此状态自然扩成 `f[j][k]`，表示体积不超过 `j`、重量不超过 `k` 时的最大价值。因为每件物品仍然只能用一次，所以两个维度都要逆序枚举，防止当前物品在同一轮里被重复使用。

这题值得形成的泛化是：**费用维度增加时，先扩状态维数；物品是否一次性，仍由循环方向决定。**
...
这题虽然讲的是愿望，但抽象出来后非常直接：每个愿望都会消耗两种资源——金钱和时间，而完成任意一个愿望的收益都一样，都是 `1`。

于是它就是二维费用 01 背包。状态表示在某个钱数和时间限制下最多能完成多少个愿望，转移时同时扣掉这两个费用，再把答案加一。

所以这题能帮助读者意识到：**价值不一定是题面给出的分数，很多时候“完成一个目标”本身就是统一价值。**
...
这题最关键的不是背“主件附件”套路，而是先把依赖关系压成有限种合法组合。因为附件不能单独买，所以围绕每个主件，真正允许出现的状态其实只有几种：不买、只买主件、主件带附件 1、主件带附件 2、主件带两个附件。

一旦把这些合法买法枚举出来，每个主件就等价于一组备选方案，而组内最多选一种，问题立刻转成分组背包。

所以这题的核心迁移点是：**有依赖的背包先别急着上树形 DP，若局部依赖规模很小，先把合法组合打包成组。**
