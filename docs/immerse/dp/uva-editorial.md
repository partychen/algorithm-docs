...
目标是“总时长尽量大但不超过 `T`”，这正是 01 背包最标准的容量模型：每首歌只能选或不选一次，容量就是剩余可用时间。

真正比普通背包多的一步只是要输出方案，因此在更新可达状态时顺手记录“这个时间点是由哪首歌转移来的”。最后从最大的可达总时长往回追，就能恢复出选歌集合。

所以这题要记住的是：**当题目要求最接近上限且要恢复方案时，先做可达性/最值背包，再补前驱记录。**
...
每次切一刀的代价取决于“当前整段”的长度，而不是切点本身，这说明决策会把一段木棍拆成左右两个独立子问题，典型地指向区间 DP。

设 `dp[i][j]` 为把切点 `i..j` 之间这整段全部切完的最小代价。若最后一次在中间某个 `k` 处切，那么左右两边会先各自切完，再额外支付整段长度 `c[j]-c[i]`。

这类题的识别方式很固定：**一次操作把区间拆成两半，且代价和当前区间长度有关。** 这通常就是“枚举最后一次操作”的区间 DP。
...
矩阵链乘之所以能做 DP，是因为你并不需要关心具体乘法过程，只要确定最后一次把整段链在哪个位置断开，左右两部分的最优代价就彼此独立。

于是 `dp[i][j]` 自然表示把第 `i` 到第 `j` 个矩阵连乘起来的最小代价。若最后在 `k` 处分裂，那么总代价就是左边 `dp[i][k]`、右边 `dp[k+1][j]`，再加上两块结果矩阵相乘的代价。

这题要沉淀的是矩阵链乘的母题意识：**区间内选择一个分界点，左右子问题独立，最后再补一笔合并代价。**
...
题目目标是先最大化“所有仓库安全值的最小值”，这类“最小值最大化”通常先二分答案。固定一个候选安全值 `S` 后，每个看守人能负责的仓库上限就被唯一确定为 `⌊p_i/S⌋`。

于是原题被翻译成一个覆盖问题：能否让这些看守人合起来覆盖至少 `N` 个仓库；若能，最小费用是多少。这里再做一次背包式 DP，`dp[j]` 表示覆盖 `j` 个仓库的最小代价。

所以这题真正的思路链是：**先把全局最小值目标二分掉，再把判定转成“容量够不够、花费最小多少”的 DP。**
...
如果从左往右推，当前决策会影响后面整条路径，不太方便同时比较字典序；反过来想，从某个格子走到最右列的最优代价，只依赖右上、正右、右下三个后继状态。

因此更自然的做法是从右往左做 DP。先求出每个格子的最小后缀代价，再在三个候选下一行中按“总代价最小、行号字典序最小”的顺序选前驱，这样路径恢复会非常干净。

这题值得记住的不是公式，而是处理字典序的方法：**先把代价 DP 做完整，再在等价最优转移中按题目要求的顺序选。**
...
每种硬币都可以无限使用，而且题目问的是“有多少种凑法”，这就是完全背包计数。关键不只是会写递推，还要理解为什么外层必须枚举硬币、内层再枚举金额。

这样做时，每种方案都会按“面额从小到大第一次被引入”的顺序被统计一次，因此不同排列不会重复计算；若把循环顺序反过来，就会把同一组硬币的不同顺序当成不同方案。

所以这题真正该记住的是：**完全背包求组合数，物品在外、容量在内。**
...
每个长方体可以旋转，所以真正的“物品”不是原始方块，而是它的 3 种摆放方式。把每种摆法都标准化成“底面长 ≥ 宽”，问题就变成这些状态之间能否上下叠放。

若一个摆法的底面长宽都严格小于另一个摆法，就可以把它放在上面，于是整题天然形成一张 DAG。接下来求最大塔高，本质就是在这张 DAG 上做最长路 DP。

这题的迁移点很重要：**带旋转的堆叠题，先枚举所有朝向，再把“能放在谁上面”转成 DAG。**
...
题目表面在比较两行文本，但真正参与匹配的单位不是字符，而是按非字母数字分隔后的单词序列。第一步先把原文本清洗成两个 token 数组，问题就瞬间降级成熟悉的序列 DP。

之后完全不用再想字符串细节，直接做 LCS 即可：`dp[i][j]` 表示前 `i` 个单词和前 `j` 个单词的最长公共子序列长度，最后一个 token 相同就接左上角，否则在上方和左方取大。

这题最值得记住的是“先预处理再 DP”的意识：**很多文本题的难点不在 DP，而在先把原始输入翻译成标准序列模型。**
...
这题和硬币计数题是同一个模型，只不过可选面额从普通硬币换成了立方数 `1^3...21^3`。每种立方数都能无限使用，目标又是统计方案数，所以直接落成完全背包计数。

状态 `dp[j]` 表示凑出和为 `j` 的方案数。按立方数从小到大更新，就能保证同一组数不会因为排列顺序不同被重复统计。

因此这题真正要迁移的是模型识别，而不是立方数这个外衣：**固定若干可重复使用的数，统计凑和方案数，就是完全背包。**
...
条码由若干段连续块组成，每段宽度在 `[1,m]`。既然题目同时限制“总长度”和“段数”，最自然的状态就是二维：长度用掉多少、已经分了多少段。

设 `dp[i][j]` 表示用 `j` 段拼出总长度 `i` 的方案数。最后一段宽度若是 `w`，那前面就必须已经用 `j-1` 段拼出 `i-w`，于是枚举最后一段宽度即可完成转移。

这题可以当作一个典型计数模板：**当对象由若干段组成，且每段长度独立受限时，常用“总量 + 段数”做双维 DP。**
...
“只能往更低处滑”这条限制非常关键，它让图中不可能出现环。于是每个格子到终点的最长长度，其实就是一张 DAG 上从该点出发的最长路。

在网格里没必要真的拓扑排序，直接记忆化搜索最自然：`dfs(i,j)` 表示从这个格子出发最多还能滑多远，递归访问四个更低的邻格并取最大值即可。

所以这题以后别只记“滑雪模板”，而要记住它的本质：**单向严格下降条件把网格变成了 DAG，记忆化搜索就是 DAG DP。**
...
取石子博弈最经典的状态就是“当前剩 `i` 个石子时，轮到当前玩家是否必胜”。因为双方都最优，所以只要存在一种合法取法能把对手送进必败态，当前态就是必胜。

于是转移非常直接：枚举每种允许取法 `a`，只要 `i-a` 合法且 `dp[i-a]=false`，就能把局面变成对手必败，因此 `dp[i]=true`。若所有后继都是必胜态，那当前自然必败。

这类题的通用心法是：**博弈 DP 先判断状态属于“存在一步送对手进败态”还是“所有走法都把对手送进胜态”。**
