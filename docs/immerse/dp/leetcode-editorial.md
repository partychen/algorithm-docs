...
这题的关键不是“偷哪一家”，而是走到第 `i` 间时只有两个互斥决定：要么完全不碰它，答案沿用前一间；要么偷它，那前一间就必须放弃，只能从前两间的最优解接过来。

因此状态自然只需要记录“处理到前 `i` 间的最优值”。因为当前选择只会依赖前一间和前两间，转移式也就直接落成 `max(dp[i-1], dp[i-2]+a[i])`。

这类题以后遇到时，先找这种“选当前就必须跳过一段”的局部排斥关系，往往就是最经典的线性 DP。
...
和无障碍版本相比，唯一变化是：有些格子根本不能落脚。于是原来的路径转移仍然成立，只是障碍格的方案数必须强制清零，因为任何路径都不允许经过它。

这也是为什么一维滚动数组依然可行：`dp[j]` 始终表示当前行走到第 `j` 列的路径数。若当前位置是障碍，就把这条“通路”直接截断成 `0`；否则再把左边的方案数累加进来。

所以这题要记住的不是一句“遇障碍置零”，而是：**障碍本质上是在 DP 网格里删掉某些状态。**
...
题目限制只能向右或向下，这意味着每个格子的最后一步来源只有两个：上面或左边。既然目标是最小路径和，那到 `(i,j)` 的最优答案一定是这两个前驱里较小的那个，再加上当前格代价。

因此这题是最标准的“路径型 DP”：不是枚举整条路径，而是把“走到当前格的最优值”当状态逐格推过去。代码里原地修改 `grid`，本质上就是把输入表直接当成 DP 表使用。

以后看到“二维网格 + 固定方向移动 + 累加代价”时，优先先问：当前格的最后一步来自哪些方向，这通常就直接决定了转移。
...
若从顶部往下想，每个点都能分叉到下面两个位置，不太像在“收答案”；但如果反过来看，从某个位置走到底部的最优值，只依赖它正下方和右下方两个子问题。

这就说明自底向上更自然：最底层本身就是答案，上一层每个点只需接住下面两个孩子里较小的那个，再加上自己的值。因为每层只会用到下一层，`dp` 压成一维也顺理成章。

这题的迁移点是：**当状态图天然是一个倒三角，且每个点只依赖下一层少数邻点时，逆推往往比顺推更干净。**
...
“凑出金额 `amount`”天然让人想到背包容量；而硬币可以重复使用，所以不是 01 背包，而是完全背包。区别不在名字，而在于同一种硬币在当前轮里还能继续被拿来更新后面的金额。

这里状态 `dp[j]` 表示凑出金额 `j` 的最少硬币数。朴素想法是枚举最后一枚硬币面值 `c`，那前面就必须已经能凑出 `j-c`，于是有 `dp[j]=min(dp[j],dp[j-c]+1)`。

所以这题最该形成的敏感度是：**当目标是“恰好凑出某值”，对象可重复使用，就优先往完全背包的“容量视角”上翻译。**
...
每个字符串一旦选了，就会同时消耗两种资源：若干个 `0` 和若干个 `1`。这正是二维费用背包的典型信号——容量不只一个，而是 `(m,n)` 这对上限。

状态 `dp[i][j]` 表示“最多用 `i` 个 0、`j` 个 1 时，最多能选多少字符串”。由于每个字符串只能取一次，所以更新时两维都必须倒序，避免同一字符串在本轮被重复计算。

以后看到“每件物品同时占用两类预算，目标是选最多/价值最大”时，就该直接联想到多维 01 背包。
...
回文子序列同时牵扯左右端点，因此单纯按长度前缀做线性 DP 不够自然。更合适的状态是一个区间：`dp[i][j]` 表示子串 `s[i..j]` 里最长回文子序列长度。

为什么转移只看两端？因为对于区间问题，左端和右端就是最关键的“新信息”。若 `s[i]==s[j]`，它们可以作为同一回文的两端，把答案建立在更小的内部区间 `dp[i+1][j-1]` 上；若不相等，至少有一个端点不会被同时选入最优解，于是退化成 `dp[i+1][j]` 与 `dp[i][j-1]` 二者取大。

这题要记住的是区间 DP 的典型信号：**答案由一段区间决定，且决策围绕左右端点展开。**
...
LCS 最核心的建模是“前缀对前缀”。因为公共子序列允许跳过字符，所以看完整串不如看 `s` 的前 `i` 个字符与 `t` 的前 `j` 个字符，最优答案怎样由更短前缀长出来。

此时分歧只在最后一个字符：若 `s[i-1]==t[j-1]`，那它们可以一起接到某个公共子序列尾部，答案从左上角 `dp[i-1][j-1]` 加一；若不相等，说明至少要舍弃一边的最后字符，于是比较“扔掉 `s` 的最后一个”与“扔掉 `t` 的最后一个”。

以后看到两个序列、允许删除但不允许打乱相对顺序时，优先把状态落在“双前缀”上，通常就是 LCS 家族。
...
编辑距离问的其实不是“怎么操作字符串”，而是“把 `s` 的前缀变成 `t` 的前缀，最少要付多少步”。一旦这样想，状态自然是 `dp[i][j]`：把 `s[0..i-1]` 变成 `t[0..j-1]` 的最小代价。

最后一步只有三种可能：删掉 `s` 的最后一个字符、在末尾插入 `t` 的最后一个字符、或者把最后一个字符替换成目标字符。若两边最后字符本来就相同，则这一步什么都不用做，直接沿用左上角状态。

所以这题最该掌握的是“最后一步分类”的套路：**看到若干编辑操作，就把状态放在前缀上，再逆推最后一步属于哪种操作。**
...
只有一次买卖时，卖出日一旦固定，最优买入日就只可能是它前面价格最低的那一天。因此不需要真的枚举买卖对，只要边扫边维护到当前位置为止的最小价格。

当扫到今天价格 `p` 时，它作为卖出点能得到的最好利润就是 `p-minPrice`；顺手再更新一下新的前缀最小值即可。整道题本质上是在维护“历史最低买入成本”和“当前最优卖出收益”。

这类股票题以后可以先问自己：若卖出时刻固定，前面的最优买入信息能否被一个简单量概括。若能，往往就不必上更重的 DP。 
