...
int dfs(int pos, int last, bool limit, bool lead) {
    ...
    for (int d = 0; d <= up; d++) {
        ...
        res += dfs(pos - 1, lead && d == 0 ? -2 : d, limit && d == up, lead && d == 0);
    }
...
$$
\frac{\text{所有点对路径费用之和}}{\binom{r-l+1}{2}}
$$

分母很好算，关键是分子。

设第 $i$ 段路（连接收费站 $i$ 和 $i+1$）的费用为 $v_i$。对于一次询问 $[l, r]$，这条边会被多少个点对经过？

- 左端点必须在 $[l, i]$ 中选，共 $i-l+1$ 种；
- 右端点必须在 $[i+1, r]$ 中选，共 $r-i$ 种。
...
$$
\sum_{i=l}^{r-1} v_i (i-l+1)(r-i)
$$

把括号展开：

$$
(i-l+1)(r-i) = -i^2 + (l+r-1)i + r(1-l)
$$

于是分子可以改写成：

$$
-\sum_{i=l}^{r-1} i^2 v_i
+ (l+r-1)\sum_{i=l}^{r-1} i v_i
+ r(1-l)\sum_{i=l}^{r-1} v_i
$$
...
都能用等差数列和、平方和公式 $O(1)$ 算出来，所以整题就变成了**线段树区间加 + 区间查询三元组**。
...
Node query(int p, int l, int r, int ql, int qr) {
    ...
    return {L.s0 + R.s0, L.s1 + R.s1, L.s2 + R.s2, 0};
}
...
int main() {
    ...
    while (m--) {
        ...
        if (op == 'C') {
            int l, r, v;
            cin >> l >> r >> v;
            update(1, 1, n - 1, l, r - 1, v);  // 修改的是路段 [l, r-1]
        } else {
            int l, r;
            cin >> l >> r;

            Node t = query(1, 1, n - 1, l, r - 1);
            long long numerator = -t.s2
                                + 1LL * (l + r - 1) * t.s1
                                + 1LL * r * (1 - l) * t.s0;
            long long denominator = 1LL * (r - l + 1) * (r - l) / 2;
...
先把“期望乘以 $n!$”翻译一下：由于当前树有 $x$ 个点时恰好有 $x$ 个可选空位，因此一共正好有 $1 \cdot 2 \cdot \cdots \cdot n = n!$ 种生长序列，而且每种等概率。题目要求的其实就是：

> **所有生长序列对应的“点对距离和”总和。**

然后还是走“按边算贡献”的经典套路。若一条边下面那棵子树大小为 $k$，那么这条边对所有点对距离和的贡献就是：
...
> **统计在所有生长序列中，子树大小恰好为 $k$ 的点一共出现了多少次。**
...
表示：在所有大小为 $i$ 的苹果树中，**子树大小恰好为 $k$ 的点的总出现次数**。

考虑从 $i-1$ 个点长到 $i$ 个点时的转移：

1. 原来子树大小就是 $k$ 的点，如果新点没有长进它的子树，它仍然贡献给 $f[i][k]$。  
   此时可选位置数为总位置数 $i$ 减去它子树内部的 $k+1$ 个空位，所以有 $i-k-1$ 种。

2. 原来子树大小为 $k-1$ 的点，如果新点长进它的子树，它会变成大小为 $k$。  
   子树内部一共有 $k$ 个可选位置，于是贡献为 $k \cdot f[i-1][k-1]$。

3. 新加入的点自己形成一个大小为 $1$ 的子树。  
   在所有大小为 $i$ 的树里，这类新增贡献一共有 $i!$ 次，所以只在 $k=1$ 时额外加上 $i!$。

于是得到递推式：

$$
f[i][k] = (i-k-1)f[i-1][k] + kf[i-1][k-1] + [k=1]\cdot i!
$$

最后答案就是：

$$
\sum_{k=1}^{n} f[n][k] \cdot k \cdot (n-k)
$$

这就是一个很标准的“组合计数意义下的 DP”，实现上只需要预处理阶乘并做二维递推即可。
