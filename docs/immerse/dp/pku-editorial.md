...
这题和上一题是同一个骨架：目标不是求一段最大子段和，而是把数组拆成左右两半，各自在自己的区间里取一段最优子数组，再把两段答案拼起来。

因此先各做一遍 Kadane 预处理最合适。`L[i]` 记录前缀 `[0,i]` 中的最大子段和，`R[i]` 记录后缀 `[i,n-1]` 中的最大子段和；最后枚举分界点 `i`，答案就是 `L[i]+R[i+1]` 的最大值。

这类题要形成的意识是：**多段最优子段和常先拆成“前缀最优 + 后缀最优”，再枚举断点合并。**
...
每种纸币数量有限，所以不能直接按完全背包处理；但目标又只是“是否能凑到某个金额”，这说明可以把每种面额的若干张纸币拆成若干个互不重复的 01 物品。

二进制拆分的作用就在这里：把 `13` 张面额 `v` 的纸币拆成 `1,2,4,6` 张四组，总组数从线性降到对数级。之后就变成普通 01 背包的可达性问题，最后从 `M` 往下找最大的可达金额。

所以这题最该记住的是：**有限件数的背包若数量较大，先二进制拆分，再转成 01 背包。**
...
题目问的是“最少补多少括号”而不是“删哪些字符”，所以最自然的对象是一段子串最终怎样被修成合法括号序列。于是状态落成 `dp[i][j]`：让 `s[i..j]` 合法所需的最少新增括号数。

转移有两类思路。若两端本来就能匹配，那可以尝试把它们配成一对，代价交给内部区间；否则就考虑把区间在某个 `k` 处分成左右两段，各自修好后拼起来。为了输出答案，再记录当前最优是“首尾配对”还是“从哪里断开”。

这题值得迁移的是区间构造套路：**先用区间 DP 求最优值，再用决策记录回溯出实际方案。**
...
合法括号子序列虽然允许跳过字符，但一旦某两个位置被放进同一个合法结构里，它们要么首尾配对，要么分别属于左右两个独立的合法片段，这正是区间 DP 的典型拆分方式。

因此设 `dp[i][j]` 表示区间 `s[i..j]` 中最长合法括号子序列长度。若 `s[i]` 与 `s[j]` 能匹配，就可以尝试把这两个端点一起选入；否则（或者除此之外）还可以把区间拆成两段，答案取 `dp[i][k]+dp[k+1][j]` 的最大值。

这题的关键识别点是：**当最优子序列结构只由“首尾是否配对”与“区间如何拆分”决定时，往往就是区间 DP。**
...
每次拿走一张中间牌时，得分只和它被拿走那一刻左右相邻的两张牌有关。这提示我们不要顺着“第一步拿谁”思考，而要改成“最后拿谁”，因为最后一步时它左右边界已经固定。

设 `dp[i][j]` 表示只剩边界牌 `i` 和 `j` 时，把中间所有牌都拿光的最小代价。若最后拿掉的是中间某张 `k`，那么在此之前 `[i,k]` 与 `[k,j]` 两边必须都已清空，最后补上一笔 `a[i]*a[k]*a[j]`。

这就是区间 DP 常见的“枚举最后一次操作”视角，和矩阵链乘、戳气球是同一家族。
...
“只能往更低的格子走”这一句直接消掉了环：高度严格下降，所以不可能绕一圈回到原点。于是整张网格图天然是一张 DAG。

在 DAG 上求从每个点出发的最长路，最顺手的方法就是记忆化 DFS。`dp[i][j]` 表示从这个格子开始最多还能滑多长，递归枚举四个更低邻居取最大，再加上当前格本身的长度 `1`。

所以这题不要只记成“滑雪模板”，而要记它背后的抽象：**单调下降约束把搜索图变成了 DAG，记忆化搜索就是在做 DAG DP。**
...
骨牌覆盖题的难点在于一块骨牌可能跨到下一行，所以只看当前行还不够，还得知道有哪些格子已经被上一行竖着放下来的骨牌占掉。

这正是状压 DP 的入口：用一个位掩码表示“当前行哪些列已被预占”。随后在当前行从左到右递归/DFS 枚举放法：若某格已被占就跳过，否则要么横放覆盖本行两个空格，要么竖放并把下一行对应位置标进新状态。

所以这题最值得掌握的是轮廓线思维：**状态记录边界上哪些位置待续，转移负责把当前行填满并生成下一行边界。**
...
同行内炮兵之间不能离得太近，跨行又会互相攻击到前两行，因此当前行能否摆放，取决于自己这一行、上一行、上上行三者的状态关系。

这就是为什么普通“只记前一行”的逐行状压不够用，必须把最近两行一起纳入状态。先预处理所有行内合法的位掩码，再枚举三行状态是否两两兼容、是否与障碍冲突，最后做三维 DP 转移。

这题的迁移点很典型：**若当前决策会和前两层甚至更多层发生局部冲突，就把这些最近层状态一起压进 DP。**
...
每一行里选哪些格子，本身就是一个长度不超过 `12` 的小集合，天然可以压成位掩码。题目限制“同行不能相邻、坏地不能选、上下相邻行同列不能同时选”，于是合法性都能用按位与快速判断。

因此按行推进最自然：`dp[i][S]` 表示处理到第 `i` 行、这一行选择状态为 `S` 的方案数。只要 `S` 本行合法，且与上一行状态 `S'` 没有同列冲突，就可以从 `dp[i-1][S']` 转移过来。

这题可以当作逐行状压 DP 的入门模板：**一旦网格宽度很小，而约束只在本行和相邻行之间传播，就优先考虑按行压状态。**
