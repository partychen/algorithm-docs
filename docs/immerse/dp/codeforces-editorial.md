...
表面上你是在原数组里选数字，实际上同一个值 `x` 的所有出现位置之间没有区别：只要决定“拿不拿值 `x`”，收益就固定是 `x * cnt[x]`。

这样原题就从杂乱序列变成了值域上的一条线：选择 `x` 会禁用 `x-1` 和 `x+1`，和经典“不相邻取数”完全同构。于是按值从小到大做 House Robber 型 DP 即可。

这题最值得学的是预处理转身：**先把相同值压成一个点，再在值域上做 DP。**
...
因为长度 `a,b,c` 的段都可以重复使用，所以这不是 0/1 背包，而是完全背包。区别只在目标不是“价值最大”，而是切出来的段数尽量多。

于是 `dp[j]` 直接表示“恰好切出长度 `j` 时最多能分成多少段”。每次尝试接上一段长度 `a/b/c`，只要前面的状态可达，就能让段数加一。

这题很适合作为完全背包变形模板：**可重复使用 + 恰好凑满 + 最大件数。**
...
这题的特殊之处在于相邻元素差必须恰好为 `1`，所以决定状态的不是下标，而是“当前值是多少”。只要知道以前有没有以 `v-1` 结尾的最优子序列，就能立刻接上一个值为 `v` 的新元素。

因此用哈希表 / map 维护 `dp[v]` 最合适：表示以数值 `v` 结尾的最长连续子序列长度。扫到一个新数 `a[i]` 时，直接令它从 `dp[a[i]-1]` 延伸而来。

这题的迁移点是：**当转移只和前一个值有关，而与具体位置关系不大时，可以把状态从“下标”改成“值”。**
...
题目要求先选出一些硬币使总和恰好为 `k`，再把这些已选硬币分成两组，其中一组和为 `x`。所以光记录“总和是多少”还不够，还得同时知道“被标记为第一组的那部分和是多少”。

于是自然得到二维背包状态：`dp[j][s]` 表示能否选出总和为 `j` 的一批硬币，并让其中某个子集和恰好为 `s`。处理一枚新硬币时，它要么被放进这批硬币但不进子集，要么既进总集合也进目标子集。

这题最值得记住的是状态补维思路：**当一个解还要再被拆成两个部分时，往往要额外开一维记录“内部子结构”的量。**
...
删除规则是“每次删一个回文子段”，这意味着一个区间最优删除方案的关键，在于某些相等元素能不能攒到同一次操作里一起删掉。

因此设 `dp[l][r]` 为删光区间 `[l,r]` 的最少次数。最朴素的想法是先单独删掉 `a[l]`，再处理右边；而如果存在和 `a[l]` 相等的元素能与它一起进入某次回文删除，就可能把操作数进一步压低。

所以这题的本质不是普通分割，而是：**区间 DP 中尝试把相同端点合并到同一次操作里。**
...
合法括号序列天然对应一棵匹配结构树：一个区间要么是外层一对括号包住内部，要么是两个合法区间的拼接。只有先看清这个结构，四色端点状态才有意义。

因为限制发生在“匹配括号至少一个有色”和“相邻括号不能同色”两类边界关系上，所以区间 DP 需要把左右端点颜色一并记住，才能在外层包裹或左右拼接时检查是否合法。

这题最该记住的不是四维数组，而是状态来源：**当约束落在区间边界上时，端点属性必须进入 DP 状态。**
...
路径价值等于节点权和减去边代价，所以从某个节点往下延伸一条链时，子树贡献若为负就不值得接上来。这样每个子节点对父亲的贡献都可以压成一个“非负净收益”。

于是树形 DP 的核心就变成：对每个点维护向下延伸的最佳单链，同时尝试把两条最优子链在当前点拼起来更新全局答案。这个结构和树上最长路径、最大路径和是同一类套路。

所以这题的关键思路是：**先把每个子树压成一条可向上提供的最优链，再在父节点处做合并。**
...
若根固定在 `v`，目标值就是所有点权乘它们到 `v` 的深度和。把根从父亲移到儿子时，儿子子树里的点都会离根更近一层，子树外的点都会更远一层，这种整体变化非常规则。

因此先用一次 DFS 求出以某个起点为根时的总答案和每棵子树的权值和，再沿树换根，就能用一个简单公式把父亲答案推到儿子答案。

这题的迁移意义很强：**当目标函数是“所有点对根的距离加权和”时，几乎就是换根 DP 的标准题型。**
...
题目同时限制了三件事：必须处在区间 `[a,b]` 内、某些位置必须/不能放数字 `d`、整个数还要对 `m` 取模为零。这样的“按位约束 + 整体模数”组合，正是数位 DP 的标准场景。

从高位到低位构造时，真正会影响后续选择的信息只有当前位置、当前余数，以及前缀是否仍贴着上界。位置奇偶性则决定这一位对数字 `d` 的取值限制。

所以这题最值得记住的是数位 DP 的三元状态来源：**位置、前缀限制、以及题目要求累计维护的全局量。**
...
因为每秒只有“再进 1 人”或“没人进入”两种结果，而人数上限又是 `N`，最自然的状态就是概率分布：`dp[t][k]` 表示 `t` 秒后恰好已有 `k` 个人进电梯的概率。

若 `k<N`，它可以由上一秒已经有 `k` 人且本秒没人进，或上一秒有 `k-1` 人且本秒恰好进 1 人转移而来；当 `k=N` 时，人数封顶后只会继续停留在满员状态。

这题的思路模板很典型：**概率过程每步只有少数离散转移时，直接对概率分布做 DP。**
