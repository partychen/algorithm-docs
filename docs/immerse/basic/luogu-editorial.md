...
这题真正需要决定的，其实只有“当前最重的纪念品该和谁同组”。因为无论如何，它都必须被尽快安排掉，否则不会对后面产生任何帮助。

若最轻的那件都能和当前最重的一起放下，那么把它们配成一组一定不亏：你已经用掉了一个最难安置的重物，同时也顺手消耗掉了一个最轻的可配对象，不会比把最重的和别的更重物品配在一起更差。

反过来，如果最轻的都放不下，说明当前最重物品不可能和任何人配对，它只能单独成组。所以这题的双指针本质是在做一个极值配对判断：**每轮先处理最重物品，并用最轻物品测试它是否还能被拯救。**
...
这题表面像区间操作，其实关键在于看“哪一部分工作是新增加出来的”。如果把道路深度看成高度曲线，那么一次区间减一，等价于在某个连续区间上削掉一层。

于是从左往右看，真正必须新开工的，只会是那些相对前一个位置“高度上升”的部分。因为平的部分可以沿用之前已经开过的施工层，下降的部分则说明前面的工程已经足够覆盖到这里了，不需要额外新增天数。

所以答案就是所有正差分之和。这题最值得记住的不是公式，而是一种观察方式：**当操作能在连续区间上统一作用时，优先看相邻差分里哪些位置代表“必须新开一层”。**
...
环形均分题最难的地方，是操作发生在边上，但你想要的是总操作次数。直接模拟每次怎么传会非常乱，所以先把每个人相对平均值的“盈亏”写出来。

设每堆减去平均值后得到差值，那么从 `1` 传到 `2`、从 `2` 传到 `3` …… 这些边上的流量其实会被前缀和唯一决定。于是原题就从“怎么传糖果”变成了“怎样选一个基准，让所有这些边流量的绝对值和最小”。

而一维绝对值和最小的基准就是中位数，所以对这些前缀和排序后取中位数即可。这题最值得记住的是：**环形均分 / 环上搬运问题，常常先转成前缀和流量，再落到中位数最优。**
...
区间调度这题最核心的不是记“按右端点排序”，而是理解为什么最早结束的区间最值得先选。因为你一旦选了某条线段，它占掉的就是从左端点到右端点这一整段时间，结束得越早，给后面留下的可用空间就越大。

如果当前有多条可以作为第一条被选中的线段，选择结束更晚的那条，只会让后续选择范围更小，不可能更优。这就是经典交换论证的直觉版本。

所以贪心策略才会如此干净：先按右端点排序，再从左到右挑所有与当前已选集合不冲突的最早结束线段。以后见到“最多选多少个互不重叠区间”，第一反应就该是这个模型。
...
这题表面是模拟排队，底层其实是最基础的多机调度模型：有若干个并行水龙头，每次来一个新任务，要把它分配给当前最早空闲的机器。

为什么这样分配最自然？因为下一位同学的开始时间完全由“哪个水龙头最早空出来”决定。若你故意把他放到更晚才空闲的水龙头，只会让完成时间更晚，不会带来任何补偿。

所以只要用小根堆维护每个水龙头当前的结束时刻，每次取出最早空闲的那个，加上当前人的接水时间，再放回去即可。它训练的是一个很通用的判断：**并行机器调度里，若任务顺序固定，就把新任务交给最早空闲的机器。**
...
这题最容易误入的想法是“要不要提前囤很多周的酸奶”。其实并不需要真的模拟库存量，因为对某一周来说，真正重要的只有一个量：**到这周为止，买一盒酸奶的最低有效成本是多少。**

这个有效成本只有两种来源：要么沿用之前某周更便宜的生产价，但每多拖一周就要额外付一次仓储费；要么干脆本周现做现卖。于是从前往后扫时，只需维护 `best = min(best + s, c[i])`。

一旦这个量确定，本周所有需求显然都该按它购买。所以这题最值得记住的，是把“是否提前生产”压缩成一个滚动最优单价，而不是显式管理库存。
...
这题是最经典的“平均等待时间最小”排序模型。关键不在于“短的人先排”这句结论，而在于理解为什么。

如果把两个相邻的人 `A`、`B` 单独拎出来比较：若 `A` 接水时间比 `B` 长，却排在 `B` 前面，那么交换他们之后，`B` 少等了 `A-B` 那么多时间，而后面的人等待总量不会增加得更糟。这个相邻交换直觉就足以推出短作业优先。

所以按接水时间升序排队后，每个人前面所有人的接水时间之和就是他的等待时间，扫一遍前缀和即可。以后碰到“最小化平均等待 / 总完成时间”的单机排序题，也应先试短作业优先。
...
这题虽然是模板题，但最该学的不是 API，而是堆适合解决哪类需求。这里所有操作都只围绕“当前最小值”展开：插入一个数、查看最小值、删除最小值。

一旦问题始终只关心“最优元素是谁”，而不关心整体有序序列长什么样，就不必维持完整排序，堆正好提供了这种局部有序性：堆顶永远是当前最小值，插入和删除堆顶都只需要对数复杂度。

所以这题真正的作用，是建立一个判断：**当问题只反复访问最值，而不需要任意位置查询时，优先考虑堆而不是平衡树或排序数组。**
...
这题和普通中位数维护相比，多了一步“输出后删除当前中位数”。但真正要维护的结构没有变：始终把序列分成左半与右半，并保证左边那一堆刚好能把当前中位数顶在堆顶。

因此仍然用大根堆存较小的一半、小根堆存较大的一半，并保持大根堆大小不少于小根堆且至多多一个。这样无论是插入新数，还是删除当前中位数，真正需要做的都只是删除 / 插入后重新平衡两边数量。

所以这题最值得学会的不是删除操作本身，而是一个维护原则：**只要中位数定义固定为“中间偏左”或“中间偏右”，就把堆大小关系固定成对应形式。**
...
中位数维护的核心不是“把数排好”，而是始终知道“左半部分有多少个、右半部分有多少个”。因为中位数只取决于这个分界点，不需要整段序列完全有序。

所以用两个堆最自然：大根堆存较小的一半，小根堆存较大的一半。只要保证大根堆大小始终不少于小根堆、且至多多一个，那么大根堆堆顶就恰好是当前中位数。

插入新数时，先根据它和分界点的大小关系决定进左边还是右边，再做一次平衡。于是模板真正维护的不是“两个堆”，而是“中位数左右两侧的元素数量平衡”。
...
这题如果把每次询问都重新排序当前前缀，显然太浪费。真正该抓住的是：询问中的第 `k` 小，随着询问顺序推进是单调增加的，因此“左边应该保留多少个最小元素”也是逐步扩大的。

于是最自然的维护方式就是把序列拆成两半：大根堆 `L` 保存当前最小的 `k` 个数，堆顶就是这 `k` 个数里最大的，也就是整体第 `k` 小；其余更大的数放进小根堆 `R`。每次新增元素后，先按大小丢到两边，再把 `L` 调整到恰好装下当前所需的最小 `k` 个。

所以这题最该形成的理解是：**不是在维护“当前排好序的前缀”，而是在维护“答案左边那一半元素”。**
...
这题本身当然可以直接调用 `sort`，但更重要的是知道为什么这里可以放心这么做。题目只要求最终升序结果，并不要求在线维护、区间修改、稳定性控制或额外统计，所以直接交给标准库排序就是最合适的选择。

`sort` 背后实现的是工程上已经充分优化过的比较排序模板，平均复杂度稳定在 `O(n log n)`，而且常数通常比手写快排更可靠。对于这种“纯排序输出题”，自己重写排序算法并不会带来额外收益。

所以这题最该建立的是工具意识：**当题目只需要离线排序结果，而没有额外结构约束时，优先用成熟库函数，不要为了模板而模板。**
...
逆序对这题最重要的不是记结论，而是看懂“为什么归并时能一次记下一大批答案”。如果右半边当前元素比左半边当前元素更小，那么它不仅和左边第一个未合并元素构成逆序，还和左边后面所有尚未合并元素都构成逆序。

这正是分治的价值：左右两半内部的逆序对可以递归解决，跨越左右两半的逆序对则在归并这一刻被统一统计出来。因为两边本来就各自有序，所以一旦右边某个元素抢先出列，左边剩余元素个数就是它新增的逆序对数量。

所以归并排序在这里不只是“顺手可用”，而是刚好把题目拆成了三部分：左边内部、右边内部、跨中点贡献。以后看到“计数对数 + 大小关系”时，也要优先想这种分治统计。
...
这题真正难的不是交换，而是先把“最优对应关系”看出来。两排火柴若想总代价最小，同长度级别的火柴应该彼此对应；把两边都按长度排序后，第 `i` 个对应第 `i` 个就是最优匹配。

匹配关系一旦确定，剩下的问题就只是在原序列里把这种对应关系调整出来。把其中一排映射成另一排中的目标位置后，题目就彻底转化成了：通过相邻交换把一个排列变成有序，最少需要多少次。

而相邻交换次数正好等于这个排列的逆序对数量。所以这题最值得迁移的思路是：**很多“最少相邻交换”题，先找目标对应关系，再把问题压成逆序对计数。**
...
如果题目只要第 `k` 小，而不要求整个序列完全有序，那么完整排序其实做多了。真正需要的是把序列划分成“三段”：比它小的、它自己、比它大的，并确保第 `k` 小那个元素落到正确位置。

`nth_element` 正好就是干这件事的：它不会把所有元素都排好，只会保证第 `k` 个位置上的数已经是最终答案，且左边都不大于它、右边都不小于它。

所以这题最值得记住的是一个复杂度意识：**当只关心第 k 小 / 第 k 大，而不需要全序结果时，选第 k 类算法通常比完整排序更对题。**
...
这题的关键不是背结论“取中位数”，而是知道为什么绝对值距离和会把答案推向中位数。把所有点排好序后，若仓库建在某个位置左边一点，左侧点会更近、右侧点会更远；继续往右移动时，这种“左边减少、右边增加”的影响会一直变化。

只有当左右两边的点数尽量平衡时，再往任一方向移动都不会让总距离继续下降，这正是中位数的位置。对于偶数个点，落在中间两个数之间的任何位置都同样最优；若答案必须取已有点，取任一中位数即可。

所以这题最该形成的直觉是：**一维绝对值距离和最小化，通常先排序，再看中位数。** 这是后面很多选址、聚会点、仓库题的共同核心。
...
曼哈顿距离和的好处，是横坐标与纵坐标可以完全拆开。也就是说，二维最优点问题其实可以分成两个独立的一维“绝对值距离和最小化”问题来做。

在一维里，最优位置由中位数控制。若点数为奇数，中位数唯一，所以这一维只有一个最优整数；若点数为偶数，那么落在中间两个数之间的所有整数都同样最优，于是这一维会形成一个闭区间。

二维答案因此就是“最优横坐标个数 × 最优纵坐标个数”。这题最值得迁移的思路是：**曼哈顿距离优化常先拆轴，再把二维题还原成两个中位数问题。**
...
要求覆盖总长度时，真正麻烦的不是单个区间，而是重叠部分不能被重复计算。所以第一步一定不是求和，而是先把有交集的区间并起来。

按左端点排序后，所有可能和当前区间重叠的段都会连续出现。于是只需维护一个当前合并段 `[L,R]`：若新区间左端点不超过 `R`，说明它和当前段有交，可以直接扩展右端点；否则当前段已经封口，可以安全结算长度并开启下一段。

所以这题最该形成的模板意识是：**区间覆盖总长度 = 排序后线性合并，只有“封口”时才结算答案。**
...
这题和很多“最大化切割长度”问题一样，第一步先看答案单调性：如果长度 `x` 可以切出至少 `k` 段，那么更短的长度当然也可以；反过来若 `x` 不行，更长只会更不行。

一旦答案有这种“可行区间前缀 / 后缀”结构，就不该暴力枚举长度，而要二分。对固定长度 `x` 来说，判定也非常直接：每根木材能贡献 `a[i] / x` 段，把它们加总即可。

所以这题最该学会的不是这一个式子，而是一个套路：**答案是某个最大可行长度，且单个材料对答案的贡献能独立计算时，通常就是二分答案 + 线性统计。**
...
题目不是问“是否能满足全部请求”，而是要找第一个出问题的位置。这类“前缀可行、后缀不可行”的结构，天然就对应二分第一个失败点。

所以先把问题改写成判定：前 `k` 个借用请求能否全部满足？若能满足，再多加请求才可能失败；若已经失败，更长前缀也不会变好，这就有了单调性。

而 `check(k)` 里之所以用差分，是因为每个请求都只是在一个区间上统一加消耗。把前 `k` 个请求全部叠到差分数组里，再扫一遍前缀和，就能知道每天实际被借走了多少间教室。这样一来，二分与差分就正好拼在一起。
...
题目要最小化“路径上经过格子的最大代价”，这类目标最适合先把答案当阈值。若你猜当前最大允许代价是 `x`，那问题就变成：只走代价不超过 `x` 的格子时，能否从第一行到达最后一行。

这个判定就不再是优化问题，而是纯可达性问题，所以 BFS 很自然。把第一行所有不超过 `x` 的格子都当成起点，看看最后一行是否有任意一个格子能被访问到。

因此这题训练的是一个非常通用的套路：**最小化路径最大值 = 二分答案 + 判定某个阈值下是否存在可行路径。**
...
这题虽然是金融背景，但本质只是一个单调方程求根。对固定月利率 `x` 来说，按“先计息、再还款”的规则模拟 `k` 个月后，最终剩余欠款是多少，是可以直接算出来的。

而且这个剩余欠款会随着利率 `x` 增大而单调上升：利率越高，同样的还款计划就越难把贷款还清。所以答案不是公式硬解，而是二分那个恰好让最终欠款逼近 `0` 的月利率。

因此这题真正训练的是一种抽象能力：**当题面给的是一个难以直接解的单调实函数方程，可以通过模拟函数值 + 浮点二分来逼近答案。**
...
锯木机高度越高，能砍下来的木材总量只会越少，因此可行性天然单调：如果某个高度还能砍够 `m`，那把机器再放低也一定可以。

这就说明答案不是靠贪心直接推出，而是要二分“最高可行高度”。判定函数也很朴素：每棵树只有超过锯木机的那一截会被砍下来，所以贡献就是 `max(0, h[i]-x)`。

所以这题和木材加工看起来不同，一个在切段，一个在截顶，但底层是同一个结构：**候选高度单调，单个对象贡献独立，整体可行性可线性统计。**
...
这题同样是“最大化段长”的标准二分模型。真正要抓住的是：如果长度 `x` 能切出至少 `k` 段，那么任何更短的长度都会更容易做到，所以答案关于“是否可行”是单调的。

对固定段长 `x`，每根绳子能切出的段数完全独立，就是 `floor(len[i]/x)`。把所有绳子的贡献加总后，只要总段数不少于 `k`，说明这个长度还可以继续尝试更大。

它和木材加工的区别只在于这里常用浮点或整数放大处理精度问题，但核心判断完全相同：**先识别单调性，再把每个材料的可贡献数量独立统计出来。**
...
这题不是直接摆放最优，而是先猜一个“最近距离下限” `x`，再问能不能把 `c` 头牛都放进去。因为如果距离 `x` 可行，那么更小的距离只会更容易，所以答案具有单调性。

判定时为什么从左到右贪心、能放就尽量放最左边是对的？因为你给当前这头牛留下的位置越靠左，后面的空位就只会越多，不会更少。这正是“局部最早放置”能推出全局可行性的原因。

所以这题最值得迁移的是：**最大化最小距离类问题，常常二分距离，再用“尽量靠左放置”的贪心做判定。**
...
这题要求“最小化最大的段和”，典型信号就是先把答案当作一个阈值去判定。若你猜当前最大段和不能超过 `x`，那问题就变成：能否把数组切成不超过 `m` 段，并且每段和都不超过 `x`。

这个判定为什么可以贪心？因为从左到右扫时，只要当前段再加一个数就会超过 `x`，那这一刀就必须切；你没有必要更早切，因为更早切只会让段数更多，不会让后面更容易。

因此 `check(x)` 就是一次线性分段统计，而答案关于可行性显然单调：阈值越大越容易切成功。以后见到“最小化最大值”的分段题，优先考虑这种“二分阈值 + 贪心判定”的组合。
...
“最大平均数”这类题最难的地方，是平均值本身不好直接 DP 或贪心；但它很适合做分数规划的标准转化。若我们猜一个答案 `x`，并把每个数都减去 `x`，那么某段区间的平均值至少为 `x`，就等价于这段新数组的区间和至少为 `0`。

于是原题被改写成判定问题：是否存在长度至少为 `m` 的子段，使和 `>= 0`。对每个右端点 `i`，只要知道所有合法左端点之前最小的前缀和 `min(s[0..i-m])`，就能判断有没有一段满足 `s[i] - min_prefix >= 0`。

所以这题最值得记住的是整条思路链：**二分平均数 → 减去猜值 → 平均数判定改写成子段和非负判定。**
...
这题不是普通“找有没有”，而是要找第一次出现的位置，所以本质上是在求一个左边界。只要数组有序，这类“边界型查找”就该优先想到二分，而不是线性扫。

`lower_bound` 恰好返回第一个“不小于 `x`”的位置，这和“第一次等于 `x` 的位置”只差一步判断：如果这个位置真的等于 `x`，它就是答案；否则说明数组里根本没有 `x`。

所以这题最值得建立的是边界意识：**很多二分题要找的不是某个值本身，而是满足条件的最左 / 最右位置。**
...
这题不需要你直接写出三次方程求根公式，而是利用连续函数的性质做数值逼近。只要一个连续函数在区间两端异号，根据介值定理，中间就一定存在一个实根。

所以做法分两步：先在题目给定范围内按固定步长粗扫，把所有发生变号的小区间找出来；再对每个小区间单独二分，把根逼近到要求精度。这里“先粗扫再二分”比直接全局二分更关键，因为三次方程可能有多个实根。

因此这题真正训练的是数值方法的建模意识：**当函数连续、根的个数有限且可通过变号定位时，可以先找含根区间，再对每个区间做二分。**
