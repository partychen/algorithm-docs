...
把数轴位置看成点，三种操作看成边，边权全为 `1`，于是题目就是无权图最短路。

用 BFS 按层扩展，第一次到达目标位置时的层数就是最优答案。
...
这是标准的单源最短路。每个棋盘格是一个点，马的八种跳法是边。由于每次跳跃代价相同，直接 BFS 即可。

一旦某个点第一次被访问，它的距离就是最短距离，不需要再次更新。
...
这题的状态是一个长度为 8 的排列。由于每次操作代价都相同，直接 BFS 即可。

为了输出方案，需要记录每个状态的前驱状态和转移操作。到达目标后从目标倒推回起点，就能恢复整条最短路径。
...
如果从起点单向 BFS，状态数会爆炸。由于题目步数上限很小，而且起点终点都已知，所以最合适的是双向 BFS。

维护从起点和终点出发的两套队列与距离表，每次扩展状态数较少的一端。一旦某个状态被两边同时访问到，就能拼出最短步数。
...
九个位置的排列状态总数是 `9! = 362880`，可以接受 BFS。为了高效判重，最常见的做法是用康托展开把每个排列映射成一个唯一整数。

这样队列里存状态编号，`dist[]`、`pre[]` 都能直接开数组，效率和实现都会更稳定。
...
这是 DLX 的原型题。DLX 的核心思想是把矩阵中的 `1` 用十字双向链表连接起来，这样删除一列和恢复一列都只需要改指针。

真正的搜索时，每次选 `1` 最少的列，这样分支数最小，通常剪枝效果最好。
...
本质和普通数独完全一样，只是规模从 `9` 变成 `16`。DLX 的优点就在于：约束模型不变，代码框架几乎不需要改，只要把约束总数和候选总数扩展即可。

先把 `A..P` 映射到 `1..16`，再按四类约束建精确覆盖矩阵。
