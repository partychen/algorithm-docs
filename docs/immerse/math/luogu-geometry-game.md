...
拉格朗日插值最重要的不是公式本身，而是先意识到：**横坐标互异的 `n` 个点，已经唯一确定了一个 `n-1` 次以下多项式。** 既然只问这个多项式在一个点 `k` 处的值，就没必要真的把系数解出来。

做法是把目标多项式拆成若干个基函数 `L_i(x)`：它在 `x_i` 处取值 `1`，在其他 `x_j` 处取值 `0`。于是答案自然是 `sum y_i * L_i(k)`。这就是为什么公式里会出现“分子乘上所有 `(k-x_j)`、分母乘上所有 `(x_i-x_j)`”。

所以这题要形成的手感是：**只求单点值时，插值不一定要求出整条多项式，直接用基函数在目标点拼出来即可。**
...
题目看起来像异或优化，真正的判定条件却是“不能出现非空子集异或和为 `0`”。这句话翻译过来，正好就是：被选出的编号向量必须线性无关。

一旦认出这是“带权线性无关集”，后面的贪心就很自然了：按权值从大到小考虑每个元素，若它能插进当前线性基，说明它带来了新的独立维度，值得保留；若插不进去，说明它已经能被前面更贵的元素张成，选它只会制造依赖。

所以这题很适合作为“线性基 = 线性无关性判定器”的模板题，而不是只把它当最大异或和工具。
...
线性基的本质，是把一堆数压成若干个“最高位互不相同”的代表元，同时不丢失它们能异或出来的所有结果。

插入时不断用已有基向量消掉当前数的最高位；若最终变成 `0`，说明它没有提供新信息；若剩下一个新的最高位，就把它作为新的基向量留下。这样每一位最多只有一个代表元。

查询最大异或和时，再从高位往低位贪心尝试：如果异或上某个基向量能让当前答案变大，就取它。因为高位优先决定数值大小，这个贪心一定正确。
...
把每个开关看成一个 `01` 向量后，按下一批开关的最终效果就是这些向量的异或和。也就是说，所有能到达的灯泡状态，恰好构成这些向量张成的异或线性空间。

线性空间里有多少个不同状态，只取决于它的维数 `rank`：每个独立基向量都有“选或不选”两种决定，因此总状态数就是 `2^rank`。所以整题真正要求的根本不是模拟开关，而是求这些向量的秩。

这题特别适合让读者建立一个结论：**异或可达状态数 = 线性基秩决定的空间大小。**
...
Nim 的关键不在背结论，而在于理解为什么“异或和”就是唯一该看的量。若当前总异或和是 `0`，那么你从任意一堆拿走若干个后，这一堆的值一定改变，于是总异或和立刻变成非零。

反过来，若总异或和非零，设它的最高位是第 `k` 位，那么一定存在某一堆在这一位上也是 `1`。把那一堆改成 `a[i] xor xr` 后，这一位会被精准消掉，而且新值更小，说明这是合法操作。于是你总能一步把局面送回异或和为 `0`。

所以 Nim 模板真正该记住的是这句博弈逻辑：**`0` 是必败态，非 `0` 一定能一步走到 `0`。**
...
反常 Nim 唯一特殊的地方在终局：普通 Nim 是“谁拿到最后一颗谁赢”，而这题变成“谁拿到最后一颗谁输”。这会让全是 `1` 的局面结论完全翻转。

若所有堆都只有 `1`，游戏就退化成轮流删堆，谁删到最后一堆谁输，所以堆数奇偶直接决定胜负；但只要还存在某堆大于 `1`，玩家就能通过调节这一堆，把局面控制回和普通 Nim 同样的结构，因此判定重新变成“异或和是否为 `0`”。

所以 `misere Nim` 最值得记的不是整张分类表，而是这条分界线：**只有“全部都是 `1`”时才与普通 Nim 不同。**
...
最近点对分治最重要的不是记住条带，而是先想清楚为什么分治后只需要关心“靠近中线”的点。左右两半分别求出的最优值是 `d`，这说明若一对跨区间点的距离还能更小，它们在横向上就不可能离中线超过 `d`。

于是只把这些候选点拎进一条宽度 `2d` 的竖条带。再按 `y` 排序后，几何上可以证明每个点只需和后面常数个点比较，因为更远的点在纵向上已经不可能更优。

所以这题真正的思路链是：**左右递归给出上界 `d`，跨区间答案只会藏在中线附近的窄条带里。**
...
加强版并没有换思路，核心仍是最近点对分治。真正需要升级的是实现细节：因为数据更大，不能在递归里反复排序，也尽量不要频繁开平方。

因此更稳的写法是全程比较距离平方，所有点只在最开始按 `x` 排一次，之后递归返回时顺手按 `y` 做归并，这样条带提取就能直接在线性时间完成。临时数组也要复用，避免常数失控。

这题要记住的是模板的工程化落地：**同一个分治思路，数据一大，性能关键就在“少排序、少开方、少拷贝”。**
...
最远点对第一步一定是先缩点集：内部点不可能比凸包上的点更远，所以问题先化成凸包直径。

接下来为什么能用旋转卡壳？因为当你固定凸包上一条边时，与它对应的“最远对踵点”沿着凸包移动具有单调性。判断依据通常写成三角形面积是否继续增大，于是指针只会往前走，不会回头。

所以这题别只记模板，要记背后的结构：**先把答案限制到凸包上，再利用对踵点的单调移动做线性扫描。**
...
最小外接矩形看起来很连续，但最优解其实有离散结构：它的一条边一定与凸包的一条边平行。于是搜索空间从“所有角度”骤减成“只枚举凸包边方向”。

对于某个固定方向，矩形的高由最远法向投影决定，左右边界由切向投影的最值决定，所以可以像旋转卡壳那样同步维护几个单调移动的指针。每转到下一条凸包边，相关最值点都只会往前走。

所以这题最重要的迁移点是：**最优包围矩形不是随便转出来的，而是被凸包边方向离散化了。**
...
Andrew 单调链的核心，不是“排完序就做两遍栈”，而是它把二维凸包拆成了两个一维单调过程。点按 `(x,y)` 排序后，下凸壳里的横坐标一定单调递增，因此可以像维护单调栈那样处理。

每加入一个新点，就检查栈顶两点与新点的转向：若不再保持凸性，说明中间那个点永远不可能出现在凸包边界上，可以直接弹掉。上凸壳同理反着做一遍。

所以这题最该记住的是凸包的局部判据：**只要最近三点转向破坏了凸性，中间点就一定没用。**
...
三维凸包和二维凸包最大的区别，是边界不再是一圈点，而是一堆三角面。增量法的核心思想是：保持当前外壳正确，然后把新点“焊”到外壳上。

若某个三角面能被新点看到，说明它已经不可能继续属于最终外壳；所有这类可见面删掉后，会留下一个由边界边组成的“洞口”。再用新点和这些边界边重新连成新三角面，外壳就被补全了。

所以三维增量法真正要理解的是这一步几何图像：**删掉面向新点的旧外壳，再沿边界圈补上一圈新面。**
...
“最省材料地包裹整个物体”这句话，本质上就是在问它的凸包表面积。因为任何凹进去的部分都不会影响最外层包裹，而真正决定外层形状的只有最外凸出的那层壳。

题目里虽然说金属条连接顶点，但这些线段的整体凸包仍然由顶点集唯一决定，因此问题没有额外模型变化，仍然是三维凸包。和模板题的差别主要只是精度要求更高。

所以这题很适合帮助读者完成一次抽象迁移：**“最小包裹面”通常就是“先取凸包，再算表面积”。**
