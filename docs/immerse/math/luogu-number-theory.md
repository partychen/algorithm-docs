...
模板就是对增广矩阵做实数域上的消元。每一列选绝对值最大的主元可以减小误差，交换到当前行后把下面各行对应列消成零。若某一列主元绝对值不足 `eps`，说明秩不满，本题直接判成无唯一解；回代后输出每个未知数。
...
这就是求 `a` 在模 `b` 下的乘法逆元。扩展欧几里得能直接求出 `a*x+b*y=gcd(a,b)=1` 的一组系数，其中 `x` 就是逆元；最后把它规范到 `1..b-1` 即可。
...
题目本身就是扩展欧几里得的定义式。递归返回 `gcd(b,a%b)` 的系数后，把 `a%b=a-a/b*b` 代回去即可完成系数转移。这里不要求最小解，只要输出任意一组即可。
...
裴蜀定理说明，所有整数线性组合恰好构成 `gcd(A_1,...,A_n)` 的倍数集合。因此能取到的最小正整数就是整组数绝对值的最大公约数。题目虽然包装成最优化，实质只剩一遍 `gcd` 累积。
...
线性筛里同时维护最小质因子结构即可。若当前数第一次被某个质数乘到，`mu` 变号；若出现平方因子，`mu` 直接置零。因为每个合数只会被最小质因子筛到一次，所以是线性的。
...
把“统计每个数有多少个约数”反过来看：每个正整数 `k` 会给它的所有倍数各贡献一次，所以总和就是 `sum floor(n/k)`。直接枚举到 `n` 能过，但更顺手的写法是整除分块，把相同的商一起算掉。
...
做法和上一题完全一致，只是这里不需要前缀和而是直接输出整张表。边界 `phi(1)=1` 一定要先补上，否则整套递推会从一开始就错位。
...
题目不是判素数，而是先把 `1..n` 的素数表整张筛出来。线性筛能在 `O(n)` 内得到按升序排列的质数数组，之后每个询问直接输出 `prime[k]`。真正卡点在数据量极大，必须把筛和输出都写成顺序数组访问。
...
上界只有 `1e5`，先把整张素数表筛出来最省事。之后遍历输入数组，遇到 `is_prime[a_i]` 为真就输出。题目规模很小，但这是“先预处理判定表、再批量过滤”的最基础用法。
...
矩阵乘法满足结合律，所以标量快速幂可以原样搬到矩阵上。维护答案矩阵为单位阵，指数按二进制拆分；当前位是 `1` 就把当前底矩阵乘进答案，然后底矩阵自乘。模板关键只有两点：乘法三重循环顺序，以及初始答案必须是单位阵。
...
最基础的二进制快速幂模板。把指数按二进制展开，遇到当前位是 `1` 就把当前底数乘进答案，然后底数平方、指数右移。因为每一步都在模 `p` 下进行，所以不会把中间数放到天文规模。
